...
__export(index_exports, {
  AnyValueObject: () => AnyValueObject,
  ApiException: () => ApiException,
  BadRequestException: () => BadRequestException,
  BaseDto: () => BaseDto,
  Body: () => Body,
  CORE_DI_SYMBOLS: () => CORE_DI_SYMBOLS,
  CommandHandlers: () => CommandHandlers,
  ConcreteDataError: () => DataError,
  Container: () => Container,
  ...
  PASSWORD_REQUIREMENTS_MESSAGE: () => PASSWORD_REQUIREMENTS_MESSAGE,
  PaginatedResourceResponse: () => PaginatedResourceResponse,
  Param: () => Param,
  ...
  buildRegistryKey: () => buildRegistryKey,
  ...
  registerQuerySchema: () => registerQuerySchema,
...
var Identifier = class {
  ...
  _uuidToInt(uuid2) {
    const buffer = import_buffer.Buffer.from(uuid2);
...
var UniqueEntityID = class extends Identifier {
  constructor(id) {
    super();
    this.value = id ? id : (0, import_uuid.v4)();
  }
};
...
var import_shallow_equal_object = require("shallow-equal-object");
var ValueObject = class {
  ...
  equals(vo) {
    ...
    return (0, import_shallow_equal_object.shallowEqual)(this.properties, vo.properties);
  }
};
...
function pickRandom(charset) {
  return charset[(0, import_crypto.randomInt)(charset.length)];
}
function generateSecurePassword(length = PASSWORD_MIN_LENGTH) {
  ...
  for (let i = chars.length - 1; i > 0; i--) {
    const j = (0, import_crypto.randomInt)(i + 1);
...
var RouteParameter = class {
  ...
  toQueryString() {
    ...
    const searchParams = entries ? new import_url.URLSearchParams(Array.from(entries)).toString() : "";
...
function buildValueSchema(field, cfg) {
  switch (cfg.type) {
    ...
    case "date" /* DATE */:
      return import_zod.z.coerce.date({
    ...
    case "enum" /* ENUM */: {
      ...
      return import_zod.z.enum(enumObj, {
        error: () => `filter[${field}]: value must be one of: ${values.join(", ")}`
      });
    }
    case "string" /* STRING */:
    default:
      return import_zod.z.string().min(1, {
...
function buildOperatorSchema(field, cfg) {
  ...
  return import_zod.z.tuple([
    import_zod.z.enum(operatorObj, {
...
function buildZodSchema(config) {
  ...
  if (config.filters && Object.keys(config.filters).length > 0) {
    shape["filter"] = import_zod.z.map(import_zod.z.string(), import_zod.z.tuple([import_zod.z.string(), import_zod.z.any()])).optional().superRefine((filterMap, ctx) => {
      if (!filterMap) return;
  ...
  if (config.includes && config.includes.length > 0) {
    ...
    shape["include"] = import_zod.z.enum(includeObj, {
      error: (ctx) => `include: "${ctx.input}" is not a supported relation. Allowed: ${config.includes.join(", ")}`
    }).optional();
  }
  if (config.fields && Object.keys(config.fields).length > 0) {
    shape["fields"] = import_zod.z.map(import_zod.z.string(), import_zod.z.array(import_zod.z.string())).optional().superRefine((fieldsMap, ctx) => {
      if (!fieldsMap) return;
  ...
  if (config.pagination !== false) {
    shape["page"] = import_zod.z.map(import_zod.z.string(), import_zod.z.string()).optional().superRefine((pageMap, ctx) => {
      if (!pageMap) return;
  ...
  if (config.sort && config.sort.length > 0) {
    const sortFieldSchema = import_zod.z.string().optional().superRefine((val, ctx) => {
    ...
    shape["sort"] = import_zod.z.array(sortFieldSchema).optional();
  }
